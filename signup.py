...
lbl5 =Label(signup_frame,text="Enter Roll No",font=("Goudy old style ",10,"bold"),fg = "#0D0B0B",bg="white").place(x=60,y=250)
roll_entrybox =Entry(signup_frame,font=("times new roman",15),bg="lightgray")
roll_entrybox.place(x=210,y=250,width=200,height=35)
...
checkbtn = ttk.Checkbutton(signup_frame, text= 'Are You Regular Student ?',variable = checkbtn_var)
...
def action():
    ...
    if roll_entrybox.get() == '':
        m_box.showerror('Error','Please Enter Your Roll Number')
    ...
    if userage > 100:
        ...
        if not myresult:
            sql = "INSERT INTO student (name, age,email,gender,rollno,Department,regular,pass) VALUES (%s,%s,%s, %s,%s,%s,%s,%s)"
            val =  (username,userage, useremail,user_gender,rol,userdep,subcribe,passs)
